...
type PublishCallback = (exchange: string, routingKey: string, body: string, contentType: string, deliveryMode: 1 | 2, headers: Record<string, string>, properties: Record<string, string>) => void;
export function initRender(store: Store, onAction: ActionCallback, onPublish: PublishCallback): void {
  ...
  const pubExchange    = document.getElementById('pub-exchange')      as HTMLInputElement;
  const pubRoutingKey  = document.getElementById('pub-routing-key')   as HTMLInputElement;
  const pubDelivery    = document.getElementById('pub-delivery-mode') as HTMLSelectElement;
  const pubContentType = document.getElementById('pub-content-type')  as HTMLSelectElement;
  const pubHeadersCt   = document.getElementById('pub-headers')       as HTMLDivElement;
  const pubPropsCt     = document.getElementById('pub-properties')    as HTMLDivElement;
  ...
  const addPropBtn     = document.getElementById('add-property')      as HTMLButtonElement;
  ...
  function addKvRow(container: HTMLDivElement, key = '', value = ''): void {
    ...
    row.innerHTML = `
      <input class="field-input kv-key" type="text" placeholder="key" value="${escHtml(key)}" autocomplete="off" />
      <span class="kv-eq">=</span>
      <input class="field-input kv-val" type="text" placeholder="value" value="${escHtml(value)}" autocomplete="off" />
  ...
  function updateInfo(): void {
    ...
    if (ex) {
      pubInfo.innerHTML = `Message will be published to exchange <strong>${escHtml(ex)}</strong>${rk ? ` with routing key <strong>${escHtml(rk)}</strong>` : ''}.`;
    } else {
      pubInfo.innerHTML = `Message will be published to the <strong>default exchange</strong> with routing key <strong>${rk ? escHtml(rk) : (pubInfoRk?.textContent ?? '')}</strong>.`;
    }
  }
  ...
  function openModal(mode: 'publish' | 'replay' = 'publish', prefill?: ModalPrefill): void {
    modalMode = mode;
    modalTitle.textContent = mode === 'replay' ? 'Replay message' : 'Publish message';
    ...
    if (data) {
      ...
      pubContentType.value = data.contentType?.includes('json') ? 'application/json' : (data.contentType || 'application/json');
      pubPayload.value     = data.body ?? '';
      Object.entries(data.headers ?? {}).forEach(([k, v]) => addKvRow(pubHeadersCt, k, String(v)));
    } else {
      pubExchange.value    = '';
      pubRoutingKey.value  = '';
  ...
  pubSend.addEventListener('click', () => {
    ...
    if (!payload) { pubPayload.focus(); return; }
    onPublish(
  ...
  document.addEventListener('keydown', (e) => {
    ...
    if (e.key === 'Enter' && (e.ctrlKey || e.metaKey) && modalOverlay.classList.contains('open')) pubSend.click();
  });
  ...
  function renderHistory(entries: HistoryEntry[]): void {
    ...
    for (const e of entries) {
      ...
      const badgeClass = e.action === 'ack' ? 'badge-ack' : e.action === 'nack-requeue' ? 'badge-nack-requeue' : 'badge-nack';
      const badgeLabel = e.action === 'ack' ? 'ACK ✓' : e.action === 'nack-requeue' ? 'NACK ↩' : 'NACK ✗';
      tr.innerHTML = `
        <td><span class="badge ${badgeClass}">${badgeLabel}</span></td>
        <td title="${escHtml(e.routingKey)}">${escHtml(e.routingKey) || '(none)'}</td>
        <td title="${escHtml(e.exchange)}">${escHtml(e.exchange) || '(default)'}</td>
        <td title="${escHtml(e.body)}">${escHtml(e.body.slice(0, 40))}${e.body.length > 40 ? '…' : ''}</td>
